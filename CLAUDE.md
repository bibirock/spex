...
`assets/rules/*.md`（`sdd-workflow` / `commands` / `testing`）是安裝到目標專案的**專案設定**，取代要使用者手寫進 `CLAUDE.md` 的章節，安裝後由專案**直接手動編輯維護**（無產生器 skill）。`commands.md` 與 `testing.md` 出廠是 TODO 樣板，由專案填入實際指令與測試工具。
