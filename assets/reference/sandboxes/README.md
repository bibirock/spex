...
## 與其他 spex 硬閘的共存（多支 PreToolUse hook）

`.claude/settings.json` 的 `hooks.PreToolUse` 底下會有多支 spex 相關的 hook，**職責各自獨立、缺一不可**——agent 版 2 支、沙盒版 3 支：

| hook | 來源 | 職責 | 觸發面 | 哪個版本 |
|---|---|---|---|---|
| `spex-stamp-guard.sh` | `spex init` 自動寫入 | 章戳鏈：保護事件流不被改寫；沙盒平面下擋「host 直接以 MCP 發含章留言」 | `Write\|Edit\|MultiEdit\|Bash\|mcp__.*` | 兩版皆有 |
| `spex-merge-guard.sh` | `spex init` 自動寫入 | PR 合併控管：擋 MCP 參數層合併、平台 CLI 合併指令、`git push` 到保護分支 | `Bash\|mcp__.*` | 兩版皆有 |
| `sandbox-guard.sh` | 本協定生成，**使用者手動合併** | 雙平面邊界：host 不得直接編輯源碼／跑碼類指令；沙盒側不得碰 git commit/push 與 az/gh/curl/wget | `Edit\|Write\|MultiEdit` 與 `Bash` | 僅沙盒版 |

共存規則（違反會直接造成假 FAIL 或防護破口）：

1. **每支都會跑**：同一次工具呼叫各 hook 獨立執行，任一 exit 2 即擋下。彼此不覆蓋、不排序依賴。
2. **合併只增不換**：把 `settings.sandbox-snippet.json` 的條目**附加**進 `hooks.PreToolUse`，絕不移除或改寫既有的 `spex-stamp-guard.sh` / `spex-merge-guard.sh` 條目。
